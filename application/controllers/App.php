...
class App extends CI_Controller{
  ...
  function index(){
    $data['todos'] = $this->model->ra_objects('todo', 'idAccess', $this->session->userdata('id'));

    if (is_array($data['todos']) || is_object($data['todos'])){
      foreach ($data['todos'] as $todo) {
        $attachments = $this->model->ra_objects('attachment', 'idTodo', $todo->id);
  ...
  function new_attachment(){
      if (!($_FILES['file']['size'] == 0)) {
        ...
        // Non sovrascrivere i file con nomi simili ma  modifica il nome
  ...
  function destroy_attachment(){
    $id = $this->uri->segment(3);
    $this->model->d_attachment('attachment', $id);
